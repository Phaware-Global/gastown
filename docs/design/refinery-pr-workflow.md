...
- **Refinery may write to**: PR-level metadata (creating the PR, posting trigger comments, advancing the merge), MR-bead state fields (`await_review_started_at`, `review_loop_iter`, `review_fix_polecat`, and `commit_sha` only via the drift-recovery side-effect described below), and origin/main (squash merge only, gated). Refinery may NOT push to a `polecat/...` branch, edit source files in-tree, or call the GraphQL `resolveReviewThread` mutation.
- **Polecat may write to**: source files in its worktree, its own polecat-namespaced branch (initial push + force-push of fix commits), review-thread state on its PR (via the GraphQL `resolveReviewThread` mutation — `gh api graphql ...`; there is no REST endpoint for thread resolution), and the MR bead's `commit_sha` field as the primary writer (via `gt done` plumbing on initial submission and on review-fix completion). Polecat may NOT call `gh pr create`, `gh pr merge`, or anything in `gt refinery pr ...`.
- **`commit_sha` shared-write rule.** The polecat is the *primary* writer of the bead's `commit_sha` (initial submission via `gt done`, review-fix completion via `gt done`). The refinery's only legitimate write is a *drift-recovery side-effect*: when `await-review` reads the bead's `commit_sha` and compares it against the upstream PR HEAD (G36 fix in PR #50), and the two disagree, the refinery updates the bead to match upstream. That write is conservative — it's resolving a state the polecat already produced (via force-push) but didn't get to record on the bead before the next patrol cycle ran. The refinery never authors a SHA the polecat hasn't already pushed.
...
1. `gt polecat checkout-branch <bead-id>` — enter the existing PR branch (idempotent against the just-shipped command from PR #49). The branch name is exactly what `gt polecat checkout-branch` constructs: `polecat/<polecat-name>-<bead-id>`. Refuses to silently swap from a different polecat branch.
...
3. Stage explicitly: `git add <each-edited-path>` for each file you touched, NOT `git add -A`. The latter would drag any incidental untracked files (test scratch, editor cruft, runtime artifacts) into the commit and silently violate the no-scope-creep contract. Then `git commit -m "<descriptive message naming the threads addressed>"`.
4. `git push --force-with-lease origin <branch-name-from-step-1>`. The polecat owns this branch; the force-push is permitted by the actor-boundary principle.
5. **Resolve the threads via the GraphQL `resolveReviewThread` mutation** — the polecat owns thread state on its own PR. Thread resolution is GraphQL-only on GitHub (there is no REST endpoint for it). Use `gh api graphql -f query='mutation { resolveReviewThread(input: {threadId: "..."}) { thread { id isResolved } } }'`. The refinery never resolves threads on the polecat's behalf; doing so muddies attribution and gives the refinery a write surface it shouldn't have.
6. `gt done` — same completion path as a feature polecat; the witness/refinery handshake takes over from there. `gt done`'s MR-update plumbing writes the new HEAD SHA into the bead's `commit_sha`, eliminating the G36 staleness on the dispatch path. (As a defense-in-depth, the refinery's drift-detecting `await-review` cycle from PR #50 will also reconcile `commit_sha` against the upstream HEAD if the polecat's update for any reason didn't land — the polecat is the primary writer, the refinery is the conservative recovery path.)
