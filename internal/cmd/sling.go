...
func init() {
	...
	// NOTE (fork-sync): upstream independently introduced --pr/--branch on `gt sling`
	// for its *resume* feature (mutually exclusive; see slingResumePR/slingResumeBranch
	// above). Our fork's *review-fix dispatch* feature also used --pr/--branch but with
	// the opposite contract (designed to be passed together). To avoid a duplicate-flag
	// init panic and preserve both features, our review-fix flags are namespaced to
	// --review-pr/--review-branch. PRODUCT DECISION pending: whether to unify these onto
	// a single flag set or keep them distinct.
	slingCmd.Flags().IntVar(&slingPR, "review-pr", 0, "Dispatch polecat to work on an existing GitHub PR for review-fix (checks out the PR's branch; pair with --review-branch)")
	slingCmd.Flags().StringVar(&slingBranchOverride, "review-branch", "", "Explicit branch for review-fix dispatch (overrides default polecat/<worker> naming; pair with --review-pr)")
...
func runSling(cmd *cobra.Command, args []string) (retErr error) {
	...
	fieldUpdates.ReviewPR = slingPR
	fieldUpdates.ReviewBranch = slingBranchOverride
	...
	if freshlySpawned {
		pane, err := startPolecatSessionFn(newPolecatInfo)
		if err != nil {
