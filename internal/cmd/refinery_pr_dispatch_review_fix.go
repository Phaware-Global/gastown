...
func buildReviewFixMission(prNumber int, threadsJSON, reviewer string) string {
	...
	b.WriteString("3. `git add -A && git commit -m \"<descriptive message naming the threads addressed>\"`.\n\n")
	b.WriteString("4. `git push --force-with-lease origin polecat/<your-name>-<bead-id>`. The polecat namespace is yours; the actor-boundary principle permits force-push here.\n\n")
	b.WriteString("5. **Resolve every thread you addressed via `gh api graphql`** (the resolveReviewThread mutation). Reply-only is not enough — gastown has no auto-resolve; the refinery's review loop cannot converge while any thread stays unresolved. This is the G13 failure mode we ship hardening against.\n\n")
