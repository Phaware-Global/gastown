...
Supported keys:
  ...
  scheduler.max_polecats_per_rig  Max concurrent polecats per rig (0 = no limit)
...
Examples:
  ...
  gt config set scheduler.max_polecats_per_rig 3
...
func runConfigSet(cmd *cobra.Command, args []string) error {
	...
	case "scheduler.max_polecats_per_rig":
		n, err := strconv.Atoi(value)
		if err != nil {
			return fmt.Errorf("invalid value for %s: %w (expected integer)", key, err)
		}
		if n < 0 {
			return fmt.Errorf("invalid value for %s: must be >= 0 (0 = no per-rig limit, N > 0 = at most N per rig)", key)
		}
		if townSettings.Scheduler == nil {
			townSettings.Scheduler = capacity.DefaultSchedulerConfig()
		}
		townSettings.Scheduler.MaxPolecatsPerRig = &n
...
func runConfigGet(cmd *cobra.Command, args []string) error {
	...
	case "scheduler.max_polecats_per_rig":
		...
		value = strconv.Itoa(scfg.GetMaxPolecatsPerRig())
