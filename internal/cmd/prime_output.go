...
func outputReviewerContext(ctx RoleContext) {
	...
	fmt.Println("- Your APPROVE does not satisfy the named pr_approver gate WHEN the rig sets pr_reviewer to your login")
	fmt.Println("  (config rejects pr_approver == pr_reviewer in that case). With pr_reviewer unset nothing checks this,")
	fmt.Println("  so assume your APPROVE may satisfy it too. Either way it DOES count toward pr_required_approvals — spend it deliberately")
