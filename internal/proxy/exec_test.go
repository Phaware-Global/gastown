...
func TestRunCommand(t *testing.T) {
	t.Run("echo world produces expected stdout", func(t *testing.T) {
		stdout, stderr, code := runCommand(context.Background(), []string{"echo", "world"}, "", nil)
	...
	t.Run("sh exit 42 returns exitCode 42", func(t *testing.T) {
		_, _, code := runCommand(context.Background(), []string{"sh", "-c", "exit 42"}, "", nil)
		assert.Equal(t, 42, code)
	})

	t.Run("stderr is captured separately", func(t *testing.T) {
		stdout, stderr, code := runCommand(context.Background(), []string{"sh", "-c", "echo err >&2"}, "", nil)
	...
	t.Run("non-existent binary returns exitCode 1", func(t *testing.T) {
		_, _, code := runCommand(context.Background(), []string{"/no/such/binary/xyzzy"}, "", nil)
	...
	t.Run("environment is restricted", func(t *testing.T) {
		...
		stdout, _, code := runCommand(context.Background(), []string{"sh", "-c", "echo ${PROXY_TEST_SENTINEL:-NOT_SET}"}, "", nil)
		assert.Equal(t, 0, code)
