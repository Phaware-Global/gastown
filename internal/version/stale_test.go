...
// commit is a DESCENDANT of the compare commit (the binary is newer than the
// build-branch ref it's run against), it is NOT reported as stale. This
// reproduces the false-positive that misled the mayor: the binary was current
// with main while the workspace's gastown checkout lagged behind, and the old
// logic warned "stale" + advised a make install that would have downgraded it.
