...
case "$cmd" in
  create)
    printf '{"id":"gt-acme-refinery","title":"Refinery for acme","status":"open"}\n'
    exit 0
    ;;
  *)
    exit 0
...
// TestEnsureRefineryAgentBead_CreatesAgentBead verifies that starting a
// refinery ensures its gt:agent bead exists via a `bd create` call carrying
...
func TestEnsureRefineryAgentBead_CreatesAgentBead(t *testing.T) {
	townRoot, err := filepath.EvalSymlinks(t.TempDir())
	if err != nil {
		t.Fatalf("EvalSymlinks: %v", err)
	}
	refineryDir := filepath.Join(townRoot, "acme", "refinery", "rig")
	...
	if err := ensureRefineryAgentBead(townRoot, refineryDir, "acme"); err != nil {
		t.Fatalf("ensureRefineryAgentBead: %v", err)
	}
	...
	if !strings.Contains(logOutput, "create --json --id=gt-acme-refinery") {
		t.Fatalf("mock bd log missing refinery agent bead create call:\n%s", logOutput)
	}
