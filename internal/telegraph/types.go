...
// L1 must return HTTP 200 to the provider on this error to prevent retry storms.
...
type NormalizedEvent struct {
	Provider     string
	EventType    string    // dot-separated: "issue.created", "comment.updated"
	EventID      string    // provider-native ID (for dedup logging only)
	Actor        string    // stable user handle
	Subject      string    // primary entity, e.g. "PROJ-1234"
	CanonicalURL string    // link back to entity in provider UI
	Text         string    // title + description snippet, or comment body
	Labels       []string  // provider-native labels (not instructions)
