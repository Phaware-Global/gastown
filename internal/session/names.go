...
// ReviewerSessionName returns the session name for a rig's Reviewer agent.
// rigPrefix is the rig's beads prefix (e.g., "gt" for gastown, "bd" for beads).
func ReviewerSessionName(rigPrefix string) string {
	return fmt.Sprintf("%s-reviewer", rigPrefix)
}
